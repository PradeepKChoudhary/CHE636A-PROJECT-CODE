...
nt=round((100000/dt),0)+1;%Number of time steps
    ...
    for g=2:nt
        for l=1:N
            if l==N
                ...
                %well at last grid block const P condition
            elseif l==1
                b(l,:)=Pres(g-1,l)*((vp*ct)/dt)+(J*pbhp);
                %well at first grid block const P condition
            else
...
grid on;
title('C(a) Pressure variation with time for last gridblock for Viscosity = ',num2str(myu));
xlabel("Time (seconds)");
ylabel("Pressure (Pa)");
hold on;
...
plot(P2_plot,Pres(nt,:),'-b');
grid on;
title('C(b) Pressure variation with x for Viscosity = ',num2str(myu));
xlabel("x (distance)");
ylabel("Pressure (Pa)");
hold off;
